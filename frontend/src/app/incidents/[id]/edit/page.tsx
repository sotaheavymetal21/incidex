...
import { Severity, Status, User, Incident } from '@/types/incident';
...
export default function EditIncidentPage() {
  ...
  const [fetchingIncident, setFetchingIncident] = useState(true);
  ...
  const fetchIncident = async () => {
    ...
    try {
      const incident: Incident = await incidentApi.getById(token!, parseInt(id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (resolvedAt && new Date(resolvedAt) < new Date(detectedAt)) {
      setError('Resolved At must be after Detected At');
      return;
    }
    ...
    try {
      ...
      await incidentApi.update(token!, parseInt(id), data);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label htmlFor="resolvedAt" className="block text-sm font-medium text-gray-700 mb-1">
                Resolved At
              </label>
              <input
                type="datetime-local"
